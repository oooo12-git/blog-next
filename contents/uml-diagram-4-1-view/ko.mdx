...
export const metadata = {
  title: "UML과 4+1 뷰 모델: 복잡한 소프트웨어 아키텍처를 꿰뚫어보는 눈 🚀",
  description:
    "UML(Unified Modeling Language)의 기본 개념부터 다양한 다이어그램 종류, 그리고 복잡한 시스템을 체계적으로 이해하게 돕는 소프트웨어 아키텍처 4+1 뷰 모델까지 알아봅니다.",
...
- **배포 다이어그램**: 소프트웨어가 어떤 하드웨어(서버, 장비)에 어떻게 배치되는지 보여줍니다.
  <Details summary="배포 다이어그램 예시 보기">
    [A sample deployment
...
- **패키지 다이어그램**: 관련된 요소들을 그룹화한 패키지들의 관계를 보여줍니다.
  {" "}
...
- **시퀀스 다이어그램**: 객체들이 메시지를 주고받으며 상호작용하는 순서를 시간의 흐름에 따라 보여줍니다.
...
- **관점**: 모든 이해관계자
- **설명**: 시스템의 핵심 기능(**사용자에 의해 인식되는 기능**)을 정의하고 다른 뷰들을 이끌어가는 시나리오입니다. "사용자가 이 시스템으로 무엇을 할 수 있는가?"에 답합니다.
