...
export async function BlogPostsBottom({
  ...
  const lowerPosts = await getPosts(locale, 3, 4);

  return (
    <div className="border-b-1 border-t-1 pt-2 pb-2">
      <div className="flex flex-col space-y-1">
        {lowerPosts.posts
          ...
          // })
          // .slice(3, 7) // 앞에서 3개의 포스트만 선택 slice(0, 3)으로 정렬된 배열에서:
          // // 3: 3번째 요소부터 시작
          // // 7: 7번째 요소 직전까지 선택 (즉, 3, 4, 5, 6 인덱스의 요소만 선택)
          .map((post) => (
            <Link
